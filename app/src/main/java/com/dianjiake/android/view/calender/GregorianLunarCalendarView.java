package com.dianjiake.android.view.calender;
...
public class GregorianLunarCalendarView extends LinearLayout implements NumberPickerView.OnValueChangeListener{
    ...
    private Calendar adjustCalendarByLimit(Calendar c, int yearStart, int yearStop, boolean isGregorian){
        ...
        if(isGregorian){
            ...
            if(yearGrego > yearStop){
                ...
                int daySway = CalenderUtil.getSumOfDayInMonthForGregorianByMonth(yearStop, MONTH_STOP_GREGORIAN);
            ...
            if(Math.abs(yearGrego - yearStart) < Math.abs(yearGrego - yearStop)){
                ...
                int daySway = CalenderUtil.getSumOfDayInMonthForLunarByMonthLunar(yearStop, MONTH_STOP_LUNAR_NORMAL);
    ...
    private void setDisplayData(boolean isGregorian){
        ...
        if(isGregorian){
            ...
            if(mDisplayYearsLunar == null){
                ...
                for(int i = 0; i < YEAR_SPAN; i++){
                    mDisplayYearsLunar[i] = CalenderUtil.getLunarNameOfYear(i + YEAR_START);
                }
            }
            if(mDisplayMonthsLunar == null){
                mDisplayMonthsLunar = new String[MONTH_SPAN_GREGORIAN];
                for(int i = 0; i < MONTH_SPAN_GREGORIAN; i++){
                    mDisplayMonthsLunar[i] = CalenderUtil.getLunarNameOfMonth(i + 1);
                }
            }
            if(mDisplayDaysLunar == null){
                mDisplayDaysLunar = new String[DAY_SPAN_LUNAR];
                for(int i = 0; i < DAY_SPAN_LUNAR; i++){
                    mDisplayDaysLunar[i] = CalenderUtil.getLunarNameOfDay(i + 1);
                }
            }
        }
    }
    ...
    private void initValuesForM(ChineseCalendar cc, boolean isGregorian, boolean anim){
        ...
        if(isGregorian){
            ...
            int monthLeap = CalenderUtil.getMonthLeapByYear(cc.get(ChineseCalendar.CHINESE_YEAR));
            if(monthLeap == 0){
                ...
                monthSway = CalenderUtil.convertMonthLunarToMonthSway(cc.get(ChineseCalendar.CHINESE_MONTH), monthLeap);
                newDisplayedVales = CalenderUtil.getLunarMonthsNamesWithLeap(monthLeap);
            }
        }
    ...
    private void initValuesForD(ChineseCalendar cc, boolean isGregorian, boolean anim){
        if(isGregorian){
            ...
            int dayStop = CalenderUtil.getSumOfDayInMonthForGregorianByMonth(cc.get(Calendar.YEAR), cc.get(Calendar.MONTH) + 1);
            ...
            int dayStop = CalenderUtil.getSumOfDayInMonthForLunarByMonthLunar(cc.get(ChineseCalendar.CHINESE_YEAR), cc.get(ChineseCalendar.CHINESE_MONTH));
    ...
    private void passiveUpdateMonthAndDay(int oldYearFix, int newYearFix, boolean isGregorian){
        ...
        if(isGregorian){
            ...
            int oldDayStop = CalenderUtil.getSumOfDayInMonth(oldYearFix, oldMonthSway, true);
            int newDayStop = CalenderUtil.getSumOfDayInMonth(newYearFix, newMonthSway, true);
            ...
            int newYearFixMonthLeap = CalenderUtil.getMonthLeapByYear(newYearFix);//1.计算当前year是否有闰月
            int oldYearFixMonthLeap = CalenderUtil.getMonthLeapByYear(oldYearFix);//2.计算之前year是否有闰月

            if(newYearFixMonthLeap == oldYearFixMonthLeap){
                //only update day picker
                newMonthSway = oldMonthSway;

                int oldMonthLunar = CalenderUtil.convertMonthSwayToMonthLunar(oldMonthSway, oldYearFixMonthLeap);
                int newMonthLunar = CalenderUtil.convertMonthSwayToMonthLunar(newMonthSway, newYearFixMonthLeap);
                int oldDayStop = CalenderUtil.getSumOfDayInMonthForLunarByMonthLunar(oldYearFix, oldMonthLunar);
                int newDayStop = CalenderUtil.getSumOfDayInMonthForLunarByMonthLunar(newYearFix, newMonthLunar);
                ...
                //月视图需要更新
                mCurrDisplayMonthsLunar = CalenderUtil.getLunarMonthsNamesWithLeap(newYearFixMonthLeap);

                //优化方案
                int oldMonthLunar = CalenderUtil.convertMonthSwayToMonthLunar(oldMonthSway, oldYearFixMonthLeap);
                int oldMonthLunarAbs = Math.abs(oldMonthLunar);
                newMonthSway = CalenderUtil.convertMonthLunarToMonthSway(oldMonthLunarAbs, newYearFixMonthLeap);
                ...
                //日视图需要更新
                int oldDayStop = CalenderUtil.getSumOfDayInMonth(oldYearFix, oldMonthSway, false);
                int newDayStop = CalenderUtil.getSumOfDayInMonth(newYearFix, newMonthSway, false);
                if(oldDayStop == newDayStop){
                    if(mOnDateChangedListener != null){
                        mOnDateChangedListener.onDateChanged(getCalendarData(newYearFix, newMonthSway, oldDaySway, isGregorian));
                    }
    ...
    private void passiveUpdateDay(int oldYear, int newYear, int oldMonth, int newMonth, boolean isGregorian){
        ...
        int oldDayStop = CalenderUtil.getSumOfDayInMonth(oldYear, oldMonth, isGregorian);
        int newDayStop = CalenderUtil.getSumOfDayInMonth(newYear, newMonth, isGregorian);
    ...
    public static class CalendarData{
        ...
        private void initChineseCalendar(){
            if(isGregorian){
                ...
                int y = pickedYear;
                int m = CalenderUtil.convertMonthSwayToMonthLunarByYear(pickedMonthSway, pickedYear);
                int d = pickedDay;
